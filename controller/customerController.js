...
const verifyEmailAddress = async (req, res) => {
  console.log("🔍 verifyEmailAddress called with body:", req.body);
  
  try {
    const isAdded = await Customer.findOne({ email: req.body.email });
    console.log("🔍 Database lookup result:", isAdded ? 'Email exists' : 'Email available');
    
    if (isAdded) {
      console.log("❌ Email already exists in database:", req.body.email);
      return res.status(403).send({
        message: "This Email already exists! Please use a different email or login.",
      });
    } else {
      console.log("✅ Email is available, generating token...");
      ...
      console.log("📧 Attempting to send verification email to:", req.body.email);
...
// Add this function to your customerController.js
const registerDirect = async (req, res) => {
  try {
    console.log("🔍 Direct registration called with:", req.body);
